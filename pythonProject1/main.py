...
springStats = pd.read_csv("D:/USUARIO/Documents/Json/Spring.csv")
summerStats = pd.read_csv("D:/USUARIO/Documents/Json/Summer.csv")
...
Kappa = "https://matchhistory.na.leagueoflegends.com/en/#match-details/ESPORTSTMNT03/1824345?gameHash=01f5ec223347dcd7&tab=overview"
KappaL = Kappa.split("/")
...
url = base_match_history_stats_url.format(KappaL[5], KappaL[6].split("?")[0], KappaL[6].split("?")[1].split("&")[0].split("=")[1])
timeline_url = base_match_history_stats_timeline_url.format(KappaL[5], KappaL[6].split("?")[0], KappaL[6].split("?")[1].split("&")[0].split("=")[1])
...
with open(KappaL[6].split("?")[0] + '.json', 'w') as outfile:
    json.dump(game_data, outfile)

with open(KappaL[6].split("?")[0] + '_timeline.json', 'w') as outfile:
            ...
            for participant in participantFrames[participantFrame]:
                ...
                for ev in events:
                    if ev['type'] == 'ITEM_PURCHASED' and ev['participantId'] == int(participant) \
                        ...
                        items.append(sightItemsNames[str(ev["itemId"])])
                    if ev['type'] == 'ITEM_DESTROYED' and ev['participantId'] == int(participant) \
                        ...
                        if items.__contains__(ev["itemId"]):
                            items.remove(sightItemsNames[str(ev["itemId"])])
...
frame = frame.drop(['teamDragon%-JGB', 'teamDragon%-MIDB', 'teamDragon%-ADCB', 'teamDragon%-SUPPB', 'teamDragon%-TOPR',
                    'teamDragon%-JGR', 'teamDragon%-MIDR', 'teamDragon%-ADCR'], axis=1)
frame = frame.drop(['teamHerald%-JGB', 'teamHerald%-MIDB', 'teamHerald%-ADCB', 'teamHerald%-SUPPB', 'teamHerald%-TOPR',
                    'teamHerald%-JGR', 'teamHerald%-MIDR', 'teamHerald%-ADCR'], axis=1)
frame = frame.drop(
    ['teamNash%-JGB', 'teamNash%-MIDB', 'teamNash%-ADCB', 'teamNash%-SUPPB', 'teamNash%-TOPR', 'teamNash%-JGR',
     'teamNash%-MIDR', 'teamNash%-ADCR'], axis=1)
...
for var_obj in array_var_obj:
    models[var_obj] = h2o.load_model("D:/USUARIO/Documents/Models/" + var_obj.split(" - ")[1] + "_BestOfFamily")
    H2Odf = h2o.H2OFrame(frame[array_var_ind])
